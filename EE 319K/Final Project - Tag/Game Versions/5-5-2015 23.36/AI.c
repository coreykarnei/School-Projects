...
void AI_Init(short aiCount, short difficulty){
	for(int i=1; (i<=aiCount && i <= characterCount); i++){
		Characters[i].bmp= &Sprite[Random()%(spriteCount-1)];						// Define sprite for Characters[i]
		Characters[i].x= Random()%(mapWidth*8-(*Characters[i].bmp).width);	// Define starting position
		Characters[i].y= Random()%(mapHeight*8-(*Characters[i].bmp).height)+(*Characters[i].bmp).height;
		Characters[i].height= (*Characters[i].bmp).height;
		Characters[i].width= (*Characters[i].bmp).width;
	//	Characters[i].spd=Random()%4+1;															// Define movement speed
		Characters[i].spd=4;															// Define movement speed
		Characters[i].mass=5;
